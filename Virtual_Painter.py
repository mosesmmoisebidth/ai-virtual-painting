...
with mp_hands.Hands(min_detection_confidence=0.85, min_tracking_confidence=0.9, max_num_hands=2) as hands:
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if len(points) != 0:
                    x1, y1 = points[8]
                    print("the coordinates position for the index finger are: {}".format((x1, y1)))
                    # Index finger
                    x2, y2 = points[12] # Middle finger
                    print("the coordinates position for the index finger are: {}".format((x2, y2)))
                    x3, y3 = points[4]  # Thumb
                    print("the coordinates position for the index finger are: {}".format((x3, y3)))
                    x4, y4 = points[20] # Pinky
                    print("the coordinates position for the index finger are: {}".format((x4, y4)))
